...
switch ($action) {
    case 'csv':
        ...
        header('Content-Disposition: attachment;filename="NetBillX_customers_' . date('Y-m-d_H_i') . '.csv"');
    ...
    case 'csv-prepaid':
        ...
        header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
        header("Content-type: text/csv");
        header('Content-Disposition: attachment;filename="NetBillX_prepaid_users' . date('Y-m-d_H_i') . '.csv"');
        header('Content-Transfer-Encoding: binary');
    ...
    case 'login':
        ...
        if ($customer) {
            ...
            // Usando concatenación
                _alert(Lang::T('You are logging in as') . " $customer[fullname],<br>don't logout just close tab.", 'info', "home", 10);

                // O usando sprintf si quieres un placeholder
                // _alert(sprintf(Lang::T('You are logging in as %s'), $customer['fullname']) . ",<br>don't logout just close tab.", 'info', "home", 10);
        }
    ...
    default:
        ...
        if (_post('export', '') == 'csv') {
            ...
            header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
            header("Content-type: text/csv");
            header('Content-Disposition: attachment;filename="NetBillX_customers_' . $filter . '_' . date('Y-m-d_H_i') . '.csv"');
            header('Content-Transfer-Encoding: binary');
