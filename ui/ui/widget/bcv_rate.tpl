        ...
        {if $bcv_rate}
            ...
            {if $bcv_history|@count > 0}
                ...
                <script>
                    ...
                    const chartLabels = [
                        {foreach $bcv_history as $d name=d}"{$d.rate_date|date_format:"%d/%m"}"{if not $smarty.foreach.d.last},{/if}{/foreach}
                    ].reverse();

                    const chartData = [
                        {foreach $bcv_history as $d name=d}{$d.rate}{if not $smarty.foreach.d.last},{/if}{/foreach}
                    ].reverse();

                    const pointColors = [
                        {foreach $bcv_history as $d name=d}
