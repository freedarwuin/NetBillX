...
switch ($action) {
    ...
    case 'app':
    // ==============================
    // Inicializar configuración DolarVzla
    // ==============================
    ...
    if (!$d) {
        ...
        $d->value = ''; // No generamos key falsa
        $d->save();
    }

    // API EXPIRATION
    $exp = ORM::for_table('tbl_appconfig')
        ->where('setting', 'dolarvzla_api_expiration')
        ->find_one();

    if (!$exp) {
        $exp = ORM::for_table('tbl_appconfig')->create();
        $exp->setting = 'dolarvzla_api_expiration';
        $exp->value = null; // Se llenará manualmente
        $exp->save();
    }
            ...
            if ($radius_enable) {
                try {
                    ...
                    $ui->assign("error_message", "Radius table not found.<br><br>" .
                        $e->getMessage() .
                        "<br><br>Download <a href=\"https://raw.githubusercontent.com/freedarwuin/NetBillX/Development/install/radius.sql\">here</a> or <a href=\"https://raw.githubusercontent.com/freedarwuin/NetBillX/master/install/radius.sql\">here</a> and import it to database.<br><br>Check config.php for radius connection details");
    ...
    case 'dbbackup':
        ...
        header('Content-Disposition: attachment;filename="NetBillX_' . count($tables) . '_tables_' . date('Y-m-d_H_i') . '.json"');
