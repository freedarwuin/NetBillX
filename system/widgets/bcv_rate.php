...
class bcv_rate
{
    public function getWidget()
    {
        ...
        if (file_exists($tmpFile)) {
            ...
            if ($data) {
                ...
                foreach ($history_for_chart as $day) {
                    ...
                    $chart_values[] = isset($day['rate']) ? (float)$day['rate'] : null;
                    $chart_euro_values[] = isset($day['eur']) ? (float)$day['eur'] : null;
                }
                ...
                if (count($chart_values) > 1) {
                    ...
                    if ($ayer > 0) $variation_percent = (($hoy - $ayer) / $ayer) * 100;
                    $variation_percent = round($variation_percent, 2);
                    if ($variation_percent > 0) $variacion_texto = "⬆ Subió {$variation_percent}%";
                    elseif ($variation_percent < 0) $variacion_texto = "⬇ Bajó {$variation_percent}%";
                    else $variacion_texto = "➖ Sin cambio";
                }
                ...
                if (count($chart_euro_values) > 1) {
                    ...
                    if ($ayer_eur > 0) $variation_percent_euro = (($hoy_eur - $ayer_eur) / $ayer_eur) * 100;
                    $variation_percent_euro = round($variation_percent_euro, 2);
                    if ($variation_percent_euro > 0) $variacion_texto_euro = "⬆ Subió {$variation_percent_euro}%";
                    elseif ($variation_percent_euro < 0) $variacion_texto_euro = "⬇ Bajó {$variation_percent_euro}%";
                    else $variacion_texto_euro = "➖ Sin cambio";
                }
            }
        }
        ...
        $ui->assign([
            ...
            'chart_values_usd'   => json_encode($chart_values),      // USD
            'chart_values_eur'   => json_encode($chart_euro_values),  // EUR
            'variacion_valor_usd' => $variation_percent,
