...
foreach (glob(File::pathFixer($PLUGIN_PATH . DIRECTORY_SEPARATOR . '*.php')) as $filename) {
    try {
        ...
        //Ignorar el error del complemento
    } catch (Exception $e) {
        //Ignorar el error del complemento
    }
}
...
// Comprobar si el usuario ha seleccionado un idioma
...
if (file_exists($lan_file)) {
    ...
    $_L['author'] = 'Generado automáticamente por NetBillX Script';
...
function _log($description, $type = '', $userid = '0')
{
    ...
    $d->userid = $userid;
    if (!empty($_SERVER['HTTP_CF_CONNECTING_IP']))   //Para comprobar si la IP es la correcta desde el túnel de Cloudflare
    {
        $d->ip = $_SERVER['HTTP_CF_CONNECTING_IP'];
    } elseif (!empty($_SERVER['HTTP_X_FORWARDED_FOR']))   //Para comprobar que la IP es la correcta desde el proxy
    {
        $d->ip = $_SERVER['HTTP_X_FORWARDED_FOR'];
    } elseif (!empty($_SERVER['HTTP_CLIENT_IP']))   //Para comprobar la IP desde internet compartido
    {
...
{
    // Definir caracteres permitidos en el código del cupón
    $characters = '0123456789';
    $charactersLength = strlen($characters);
    $vouchers = array();

    // Intenta generar códigos de cupón únicos
    for ($j = 0; $j < $totalVouchers; $j++) {
        do {
            ...
            // Comprueba si el código de cupón generado ya existe en la matriz
